...
time_checks <- function(end_date, start_date, time_step, output_frequency) {
  ...
  if (checks_passed && !(output_frequency %in% list("week", "month", "day",
                                                    "year", "time_step"))) {
    checks_passed <- FALSE
  ...
  if (checks_passed) {
    outs <- list(checks_passed, number_of_time_steps, number_of_years,
                 number_of_outputs, output_frequency)
    names(outs) <- c("checks_passed", "number_of_time_steps",
...
bayesian_checks <- function(prior,
  ...
  if ((is(prior, "matrix") && nrow(prior) == 1) || (is(prior, "numeric") &&
    ...
    priors <- round(rnorm(count, start_priors, sd_priors) / round_to,
                    digits = round_to_digits) * round_to
  ...
  rates <- data.frame(rate = round(seq(min_rate, max_rate, step_size),
                                   digits = round_to_digits),
                      prior_probability = rep(0,
                                              length(seq(min_rate,
                                                         max_rate,
                                                         step_size))),
...
movement_checks <- function(x, rast, start_date, end_date) {
  ...
  if (checks_passed) {
    ...
    movement_from <- SpatialPointsDataFrame(moves[, 1:2],
                                            ...
                                            proj4string =
                                              CRS("+init=epsg:4326"))
    movement_to <- SpatialPointsDataFrame(moves[, 3:4], data = moves,
    ...
    movements <- data.frame(row_from = rowcol_from[, 1],
...
parameter_checks <- function(n, parameter_means, parameter_cov_matrix) {
  ...
  if (checks_passed) {
    parameters <- data.frame(MASS::mvrnorm(n, parameter_means,
