...
quantity_allocation_disagreement <-
  function(reference,
    ...
    # Original comparison and references rasters
    comp <- comparison
    ref <- reference
    
    #Create reclassification matrices
    rclmat_comp <- matrix(c(1, Inf, 1, 0, 1, 0, NA, 0, 0), ncol = 3, byrow = TRUE)
    rclmat_ref <- matrix(c(NA, 0, 2, 1, Inf, 1, 0, 0.99, 0),
                         ncol = 3,
                         byrow = TRUE)
    ...
    if (use_configuration) {
      if (sum(terra::values(comparison) > 0, na.rm = TRUE) == 0) {
        ...
        if (np_comp > 1) {
          enn_mn_comps <-
            suppressWarnings(landscapemetrics::lsm_c_enn_mn(
              comparison,
              directions = 8,
              verbose = TRUE
            ))
    ...
    if (is.nan(tp_fp) || tp_fp == 0) {
      tp_fp <- 1
    }
    if (is.nan(tp_fn) || tp_fn == 0) {
      tp_fn <- 1
    }
    if (is.nan(tn_fp) || tn_fp == 0) {
      tn_fp <- 1
    }
    if (is.nan(tn_fn) || tn_fn == 0) {
      tn_fn <- 1
    }
    ...
    if (use_distance) {
      ...
      distance_differences <- unlist(distance_differences,
                                     recursive = TRUE,
                                     use.names = TRUE)
