...
configuration_qad_lite <- function(reference,
  ...
  ref_values_fn <- adjust_path(gsub(".tif", "_qad_reclassed_vals.rds", reference),
                                                        output_folder_path)
  config_qad_lite$ref_points <-
    adjust_path(gsub(".tif", "_qad_masked_positive_pts.gpkg", reference), output_folder_path)
  ...
  if (use_distance) {
    ...
    config_qad_lite$distance_differences <-
      lapply(config_qad_lite$distance_differences, all_distances)
    config_qad_lite$distance_differences <-
      unlist(config_qad_lite$distance_differences, recursive = TRUE, use.names = TRUE)
  }
