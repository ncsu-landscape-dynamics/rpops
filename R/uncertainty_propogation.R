...
choose_variable_based_on_probability <- function(x, n = 1) {
  ...
  for (i in seq_len(nrow(x))) {
    ...
    if (i == nrow(x)) {
      if ((x$bin[i] < 1.00 && x$bin[i] > 0.985) ||
          (x$bin[i] > 1.00 && x$bin[i] < 1.01)) {
        x$bin[i] <- 1.00
      }
    }
  }
...
uncertainty_check <- function(priors, round_to = 0, n = 1) {
  ...
  if (class(priors) %in% c("matrix", "data.frame") && ncol(priors) == 2) {
    ...
    } else if (class(priors) %in% c("matrix", "data.frame") &&
               nrow(priors) >= 1) {
      if (class(priors) == "matrix") {
        priors <- data.frame(priors)
      }
      if (round(sum(priors[, 2]), 3) < 0.985 ||
          round(sum(priors[, 2]), 3) > 1.01) {
        checks_passed <- FALSE
